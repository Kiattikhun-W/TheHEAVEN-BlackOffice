...
import { RepairPrivateModel, } from "../../../models"
...
class View extends React.Component {
  ...
  _fetchData = () => this.setState({ loading: true, }, async () => {
    ...
    const repair_privates = await repair_private_model.getRepairPrivateBy({
      page: 1,
      limit: 10,
    ...
    this.setState({
      loading: false,
      repair_privates,
      dataSource: repair_privates?.result[0],
      paginationDatas: {
        total: repair_privates?.result[0]?.total,
      }
    })
  })
  ...
  _onDelete = (code) => Swal.fire({
    ...
    const res = await repair_private_model.deleteRepairPrivateByCode(Number(code))
    if (res.code === 200) {
  ...
  handleChangePagination = async (page, pageSize) => {
    try {
      ...
      const res = await await repair_private_model.getRepairPrivateBy({
        page: page === 0 ? 1 : page,
        limit: pageSize,
        propertyId: property,
      });
      ...
      <div>
        ...
        <Card className="cardTable">
          <CardHeader style={{ backgroundColor: '#634ae2' }}>
            <h3 className="text-header text-white">เพิ่มรายการแจ้งซ่อม </h3>
            {permission_add && (
              <Link
                to={`/properties/${idproperty}/repairprivate/insert`}
                className="btn btn-success float-right"
              >
                <i className="fa fa-plus" aria-hidden="true" /> แจ้งซ่อมพื้นที่ส่วนตัว
              </Link>
            )}
          </CardHeader>
          <CardBody>
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  // ellipsis: true,                  
                  align: 'center',
                  render: (cell) => cell.unit.unitName ? cell.unit.unitName : null
                },
                {
                  title: "รายละเอียด",
...

                {
                  ...
                  render: (cell) => {
                    ...
                    if (permission_edit) {
                      row_accessible.push(
                        <Link key="update" to={`/properties/${idproperty}/repairprivate/update/${cell.id}`} title="แก้ไขรายการ">
