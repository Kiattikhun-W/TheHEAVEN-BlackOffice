...
class View extends React.Component {
  ...
  _fetchData = () => this.setState({ loading: true, }, async () => {
    ...
    // console.log(repair_zones)
    this.setState({
      loading: false,
      repair_zones: repair_zones.result[0],
      dataSource: repair_zones.result[0],
      paginationDatas: {
        total: repair_zones.result[0].total,
      },
    })
  })
  ...
  _onDelete = (code) => Swal.fire({
    ...
    const res = await repair_zone_model.deleteRepairZoneByCode(Number(code))
    if (res.code === 200) {
  ...
  handleChangePagination = async (page, pageSize) => {
    try {
      ...
      const res = await repair_zone_model.getRepairZoneBy({
        page: page === 0 ? 1 : page,
        limit: pageSize,
      });
      ...
      <div>
        ...
        <Card className="cardTable">
          <CardHeader style={{ backgroundColor: '#634ae2' }}>
            <h3 className="text-header text-white">พื้นที่ซ่อม </h3>
            {permission_add &&
              <Link to={`/setting/repair-zone/insert`} className="btn btn-success float-right" >
                <i className="fa fa-plus" aria-hidden="true" /> เพิ่มพื้นที่ซ่อม
              </Link>
            }
          </CardHeader>
            ...
            <DataTable
              ...
              columns={[
...

                {
                  ...
                  // width: 120,
                  render: (cell) => {
                    return cell === true ? <Tag color="success">Yes</Tag> : <Tag color="error">No</Tag>
                  }
                },
                {
                  title: "พื้นที่ส่วนกลาง",
                  dataIndex: "useInCommonArea",
                  // filters: [
                  //   { text: 'ใช้งาน', value: true, },
                  //   { text: 'ไม่ใช้งาน', value: false, },
                  // ],
...

                {
                  ...
                  render: (cell) => {
                    ...
                    if (permission_edit) {
                      row_accessible.push(
                        <Link key="update" to={`/setting/repair-zone/update/${cell.id}`} title="แก้ไขรายการ">
